...
class LeagueData():
    def __init__(self, leagueID, teamID, week):
        self.leagueID = leagueID
        self.teamID = teamID
        self.week = week

    def get_players_data(self, page):
        '''
        Get players from ESPN with stats depending on page (url)
        page: {clubhouse, freeagency}
        '''
        url = 'http://games.espn.com/fba/{}?leagueId={}&teamId={}&seasonId=2018'.format(page, self.leagueID, self.teamID)
    ...
    def get_all_teams(self):
        ...
        '''
        url = 'http://games.espn.com/fba/leaguerosters?leagueId={}'.format(self.leagueID)
        source_code = requests.get(url)
        plain_text = source_code.text
        soup = BeautifulSoup(plain_text, 'lxml')
    ...
    def get_all_teams_stats(self):
        ...
        '''
        url = 'http://games.espn.com/fba/scoreboard?leagueId={}&matchupPeriodId={}'.format(self.leagueID, self.week-1)
        # url = 'http://games.espn.com/fba/scoreboard?leagueId={}&seasonId=2018'.format(self.leagueID)
        source_code = requests.get(url)
        plain_text = source_code.text
        soup = BeautifulSoup(plain_text, 'lxml')
        teams = []
    ...
    def get_player_universe(self):
        ...
        '''
        free_agents = self.get_players_data('freeagency')
        my_team = self.get_players_data('clubhouse')
...
if __name__ == '__main__':
    # test 
    myLeague = LeagueData(leagueID='204515', teamID='12', week=5)
    print( myLeague.get_player_universe() )